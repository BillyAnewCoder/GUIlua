local drawing = {} do
    ...
    local ENABLE_PERFORMANCE_METRICS = true

    local Signal = {}
    Signal.__index = Signal
    Signal.ClassName = "EnhancedSignal"
    Signal._connections = 0
    ...
    function Signal.new()
        ...
        -- Enhanced memory management with performance tracking
        self._cleanupConnection = self._bindableEvent.Event:Connect(function(key)
            self._argMap[key] = nil

            -- Performance metrics
            if ENABLE_PERFORMANCE_METRICS then
        ...
        -- Track total signal instances
        Signal._connections = Signal._connections + 1
    ...
    function Signal:Fire(...)
        ...
        if ENABLE_PERFORMANCE_METRICS and startTime then
            self._lastFireDuration = tick() - startTime
        end
    end
    ...
    function Signal:Connect(handler)
        if not (type(handler) == "function") then
            error(("Enhanced Signal connect(%s) - handler must be a function"):format(typeof(handler)), 2)
        end
        ...
        local connection = self._bindableEvent.Event:Connect(function(key)
            ...
            if args then
                ...
                if ENABLE_TRACEBACK then
                    warn("Enhanced Signal: Missing arg data, possible reentrance issue")
                end
            end
    ...
    local function ismouseover(obj)
        ...
        if mousepos.X >= (posX - tolerance) and mousepos.Y >= (posY - tolerance) and 
           mousepos.X <= (sizeX + tolerance) and mousepos.Y <= (sizeY + tolerance) then
            return true
        end
    ...
    local function ismouseoveradvanced(obj, customBounds)
        local bounds = customBounds or {
            minX = obj.Position.X,
            minY = obj.Position.Y,
            maxX = obj.Position.X + obj.Size.X,
            maxY = obj.Position.Y + obj.Size.Y
        }
        
        local mousepos = services.InputService:GetMouseLocation()
        
        return mousepos.X >= bounds.minX and mousepos.Y >= bounds.minY and 
               mousepos.X <= bounds.maxX and mousepos.Y <= bounds.maxY
    end

    -- Enhanced UDim2 to Vector2 conversion with caching for performance
    local udim2Cache = {}
    local function udim2tovector2(udim2, vec2)
        -- Create cache key for frequently used conversions
        local cacheKey = string.format("%.2f,%.2f,%.2f,%.2f,%.2f,%.2f", 
            udim2.X.Scale, udim2.X.Offset, udim2.Y.Scale, udim2.Y.Offset, vec2.X, vec2.Y)
        
        if udim2Cache[cacheKey] then
            return udim2Cache[cacheKey]
        end
        ...
        if #udim2Cache < 1000 then
            udim2Cache[cacheKey] = newvec2
        end
    ...
    local function clearUDim2Cache()
        udim2Cache = {}
    end
    ...
    -- Enhanced object management with performance optimizations
    local objchildren = {}
    local objmts = {}
    local objvisibles = {}
    local mtobjs = {}
    ...
    -- NEW: Advanced features for enhanced UI library
    local objanimations = {} -- Store animation data for smooth transitions
    local objthemes = {} -- Individual object theming support
    local objtooltips = {} -- Tooltip system for better UX
    local objhotkeys = {} -- Hotkey bindings for UI elements
    local objperformance = {} -- Performance metrics per object
    local objeffects = {} -- Visual effects like shadows, glows, etc.
    ...
    -- Enhanced drawing constructor with advanced features
    function drawing:new(shape)
        ...
        -- Initialize enhanced features for this object
        objconnections[obj] = {}
        objanimations[obj] = {}
        objthemes[obj] = {}
        objtooltips[obj] = nil
...
-- Enhanced utility system with performance optimizations
local utility = {
    -- Performance tracking
    performance = {
        totalFrames = 0,
...
function utility.dragify(object, dragoutline, options)
    options = options or {}
    local start, objectposition, dragging, currentpos
    local constraints = options.constraints or {}
    local smoothing = options.smoothing or false
    local snapToGrid = options.snapToGrid or false
    ...
    utility.connect(services.InputService.InputChanged, function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement and dragging then
            local newX = objectposition.X.Offset + (input.Position - start).X
            local newY = objectposition.Y.Offset + (input.Position - start).Y
            
            -- Apply constraints
            if constraints.minX then newX = math.max(newX, constraints.minX) end
            if constraints.maxX then newX = math.min(newX, constraints.maxX) end
            if constraints.minY then newY = math.max(newY, constraints.minY) end
            if constraints.maxY then newY = math.min(newY, constraints.maxY) end
            ...
            currentpos = UDim2.new(objectposition.X.Scale, newX, objectposition.Y.Scale, newY)
    ...
    utility.connect(services.InputService.InputEnded, function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 and dragging then 
            ...
            if smoothing then
                utility.animate(object, {
                    Position = currentpos
                }, 0.2, "Quart", "Out", function()
...
function utility.animate(object, properties, duration, easingStyle, easingDirection, callback)
    easingStyle = easingStyle or "Quad"
    easingDirection = easingDirection or "Out"
    duration = duration or 0.3
    
    local animationId = #utility.animations + 1
    ...
    connection = runService.Heartbeat:Connect(function()
        ...
        -- Apply easing
        local easedProgress = utility.ease(progress, easingStyle, easingDirection)
        
        -- Interpolate properties
        for prop, endValue in pairs(properties) do
            local startValue = startProperties[prop]
            if typeof(startValue) == "Vector2" and typeof(endValue) == "Vector2" then
                object[prop] = startValue:lerp(endValue, easedProgress)
            elseif typeof(startValue) == "Color3" and typeof(endValue) == "Color3" then
                object[prop] = startValue:lerp(endValue, easedProgress)
            elseif typeof(startValue) == "number" and typeof(endValue) == "number" then
                object[prop] = startValue + (endValue - startValue) * easedProgress
        ...
        if progress >= 1 then
            ...
            utility.animations[animationId] = nil
            if callback then callback() end
        end
    end)
    
    utility.animations[animationId] = {
...
-- NEW: Easing functions for smooth animations
function utility.ease(t, style, direction)
    ...
    elseif style == "Quart" then
        ...
        elseif direction == "InOut" then
            return t < 0.5 and 8 * t * t * t * t or 1 - 8 * (1 - t) * (1 - t) * (1 - t) * (1 - t)
        end
    end
...
-- Enhanced text measurement with caching for better performance
local textCache = {}
function utility.textlength(str, font, fontsize)
    local cacheKey = string.format("%s|%s|%d", str, tostring(font), fontsize)
    ...
    if #textCache < 500 then
        textCache[cacheKey] = textbounds
    end

    return textbounds
end

-- NEW: Advanced theme management system
...
function utility.createCustomTheme(name, colors)
    utility.themes[name] = colors
end

function utility.getThemeColor(colorKey)
    local theme = utility.themes[utility.currentTheme]
    return theme and theme[colorKey] or Color3.fromRGB(255, 255, 255)
end

function utility.onThemeChanged(themeName)
    -- Broadcast theme change to all themed objects
    for obj, themeData in pairs(objthemes) do
        if objexists[obj] and themeData.applyTheme then
            themeData.applyTheme(themeName)
        end
    end
end

-- NEW: Performance monitoring system
function utility.startPerformanceMonitoring()
    ...
    local connection = runService.Heartbeat:Connect(function()
        ...
        if now - lastUpdate >= 1 then -- Update every second
            utility.performance.averageFPS = frameCount / (now - lastUpdate)
            utility.performance.totalFrames = utility.performance.totalFrames + frameCount
            utility.performance.lastFPSUpdate = now
            utility.performance.memoryUsage = collectgarbage("count")
            
            frameCount = 0
            lastUpdate = now
            
            -- Warn about performance issues
            if utility.performance.averageFPS < 30 then
                warn("Enhanced UI Library: Low FPS detected (" .. math.floor(utility.performance.averageFPS) .. " FPS)")
            end
        end
    end)
...
function utility.cleanupAnimations()
    local cleaned = 0
    for id, anim in pairs(utility.animations) do
        if not objexists[anim.object] then
            if anim.connection then
                anim.connection:Disconnect()
            end
            utility.animations[id] = nil
            cleaned = cleaned + 1
        end
    end
    return cleaned
...
-- NEW: Sound effect system for enhanced user experience
function utility.playSound(soundType, volume)
    volume = volume or 0.5
    local soundId = ""
    
    -- Sound library for UI interactions
    local sounds = {
        click = "131961136", -- Click sound
        hover = "131961102", -- Hover sound  
        error = "131961136", -- Error sound
        success = "131961136", -- Success sound
        slide = "131961136", -- Slider/drag sound
        toggle = "131961136", -- Toggle sound
        notification = "131961136" -- Notification sound
    }
    
    soundId = sounds[soundType] or sounds.click
...
-- NEW: Tooltip system for better user experience
function utility.createTooltip(object, text, options)
    options = options or {}
    local delay = options.delay or 0.5
    local offset = options.offset or Vector2.new(10, -25)
    local backgroundColor = options.backgroundColor or utility.getThemeColor("surface")
    local textColor = options.textColor or utility.getThemeColor("text")
    
    local tooltip
    local isHovering = false
    local showConnection
    
    object.MouseEnter:Connect(function()
        isHovering = true
        
        -- Delayed tooltip display
        showConnection = task.wait(delay)
        if isHovering then
            tooltip = utility.create("Square", {
                Size = UDim2.new(0, #text * 7 + 20, 0, 25),
                Position = UDim2.new(0, services.InputService:GetMouseLocation().X + offset.X, 0, services.InputService:GetMouseLocation().Y + offset.Y),
            ...
            -- Smooth fade in
            utility.animate(tooltip, {Transparency = 0}, 0.2, "Quad", "Out")
        end
    end)
    ...
    object.MouseLeave:Connect(function()
        ...
        if showConnection then
            task.cancel(showConnection)
        end
        if tooltip then
            utility.animate(tooltip, {Transparency = 1}, 0.2, "Quad", "Out", function()
                tooltip:Remove()
                tooltip = nil
            end)
        end
...
function utility.showNotification(title, message, duration, notificationType)
    duration = duration or 3
    notificationType = notificationType or "info"
    
    local colors = {
        info = {bg = Color3.fromRGB(70, 130, 255), text = Color3.white},
        success = {bg = Color3.fromRGB(80, 200, 120), text = Color3.white},
        warning = {bg = Color3.fromRGB(255, 180, 0), text = Color3.black},
        error = {bg = Color3.fromRGB(255, 100, 100), text = Color3.white}
    }
...
utility.startPerformanceMonitoring()

library.version = "2.0.0-Enhanced"
library.author = "Enhanced Version"
library.utility = utility
library.animate = utility.animate
library.playSound = utility.playSound
library.createTooltip = utility.createTooltip
...
function library:cleanup()
    ...
    for obj, connections in pairs(objconnections) do
